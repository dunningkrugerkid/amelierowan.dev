...
const $$CatGallery = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4"> ${displayPhotos.map((photo) => renderTemplate`<div class="aspect-square rounded-lg overflow-hidden bg-pink-50 border border-pink-100 group hover:border-pink-300 transition-colors"> ${photo.url ? renderTemplate`<img${addAttribute(photo.url, "src")}${addAttribute(photo.alt, "alt")} loading="lazy" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">` : renderTemplate`<div class="w-full h-full flex items-center justify-center group-hover:bg-pink-100 transition-colors"> <span class="text-pink-300">${photo.alt}</span> </div>`} </div>`)} </div>`;
}, "/Users/rowan/amelierowan.dev/src/components/content/CatGallery.astro", void 0);

const $$Cat = createComponent(($$result, $$props, $$slots) => {
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "my cat | rowan" }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<main class="max-w-6xl mx-auto px-4 py-8"> <h1 class="text-4xl font-bold mb-2 text-gray-800">my cat</h1> <p class="text-gray-600 mb-8">
here's my cat! photos powered by cloudinary.
</p> ${renderComponent($$result2, "CatGallery", $$CatGallery, { "photos": [
  ...
  ] })} <div class="mt-8 p-6 bg-pink-50 rounded-lg border border-pink-200"> <h2 class="text-lg font-semibold mb-2 text-gray-800">want to add photos?</h2> <p class="text-gray-600 text-sm">
upload your cat photos to cloudinary in the <code class="bg-pink-100 px-1 rounded">cat-photos</code> folder
